...
export default function SliderGameStats({userGameStats}:Props) {
    ...
    return (
        <>
            <Swiper onSwiper={(swiper: any) => (swiperRef.current = swiper)} slidesPerView={1} pagination={{ clickable: true }} className='w-full rounded-2xl mt-6 border border-gray-600'>
                ...
                <SwiperSlide>
                    <div className='flex flex-col  justify-center bg-gradient-to-b from-black via-gray-900 to-black px-12 py-10'>
                        <div className="flex ">
                            {userGameStats.topGames.map((item:any, index:number) => (
                                <Link key={index} href={`/gamePage/${item.game_id}`} className='group relative mr-4 flex justify-center items-center cursor-pointer w-16 h-21 sm:w-24 sm:h-32 md:w-32 md:h-48 2xl:w-48 2xl:h-64 transition hover:scale-110'>
                                    ...
                                    <p className='absolute bottom-[-5%] bg-zinc-900 border border-gray-600 px-4 rounded text-xs md:text-sm'>{item.hours_played} h</p>
                                </Link>
                                ))}
                        </div>
                        <p className='text-xl mt-6'>Top games played</p>
                    </div>
                ...
                <SwiperSlide>
                    <div className='flex flex-col justify-center bg-gradient-to-b from-black via-gray-900 to-black px-12 py-10'>
                        ...
                        <p className='text-xl mt-6'>Fav games</p>
                    </div>
                </SwiperSlide>
            </Swiper>
        </>
