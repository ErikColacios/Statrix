    ...
    return (
        <>
            <Swiper onSwiper={(swiper: any) => (swiperRef.current = swiper)} slidesPerView={1} pagination={{ clickable: true }} className='w-full rounded-2xl mt-6'>
                ...
                <SwiperSlide>
                    <div className='flex flex-col items-center bg-linear-to-b from-black via-gray-900 to-black px-3 md:px-10 py-6 h-68 2xl:h-96'>
                        ...
                        <div className="flex ">
                            {userGameStats.topGames.map((item:any, index:number) => (
                                <Link key={index} href={`/gamePage/${item.game_id}`} className='group relative mr-4 flex justify-center items-center cursor-pointer w-16 h-21 sm:w-24 sm:h-32 md:w-32 md:h-48 2xl:w-48 2xl:h-64 transition hover:scale-110'>
                                    <img src={item.game_base_image} className='w-full h-full rounded-lg transition duration-300 group-hover:blur-xs group-hover:brightness-50' alt='Game cover'/>
                                    ...
                                    <p className='absolute bottom-[-5%] bg-zinc-900 border border-gray-600 px-1 md:px-4 rounded-sm text-xs md:text-sm'>{item.hours_played} h</p>
                                </Link>
                                ))}
                        </div>
                    </div>
                ...
                <SwiperSlide>
                    <div className='flex flex-col items-center bg-linear-to-b from-black via-gray-900 to-black px-12 py-6 h-68 2xl:h-96'>
                        ...
                        <div className="flex">
                            {userGameStats.favGames.map((item:any, index:number) => (
                                <Link key={index} href={`/gamePage/${item.game_id}`} className='group relative mr-4 flex justify-center items-center cursor-pointer w-16 h-21 sm:w-24 sm:h-32 md:w-32 md:h-48 2xl:w-48 2xl:h-64 transition hover:scale-110'>
                                    ...
                                    <p className='absolute bottom-[-5%] bg-zinc-900 border border-gray-600 px-4 rounded-sm text-xs md:text-sm'>{index +1}</p>
                                </Link>
                                ))}
                        </div>
                    </div>
