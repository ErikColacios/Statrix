...
export default function SettingsModals({ chooseMode, currentAvatarId, currentBannerId }: SettingsModalsProps) {
  ...
  if (chooseMode === "avatar") {
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center h-[45vh] border border-gray-600 space-y-4 px-4 md:px-10 blur-none text-white rounded-2xl bg-black/20 backdrop-blur-lg">
        <Dialog.Close className="mt-8 absolute top-0 right-10 p-2 rounded transition hover:bg-gray-800" >
            ...
            <input type="text" id="deleteAccountInput" name="deleteAccountInput" maxLength={20} className="w-1/4 p-1 text-center bg-gray-800 outline-none border border-1 border-gray-700 focus:border-green-600" />

            <div className="flex space-x-8 mt-12">
                <button className="text-md sm:text-lg border-green-500 text-green-400 hover:bg-green-900/30 rounded-xl px-6 py-3">Delete</button>
