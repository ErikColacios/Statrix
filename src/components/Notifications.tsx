...
export default function Notifications({ userId, notificationCount }: Props) {
    ...
    return (
        <div ref={dropdownRef}>
            <button className={`relative rounded-full p-2 hover:bg-zinc-700 cursor-pointer ${dropdown && "bg-zinc-900"}`} onClick={() => setDropdown(!dropdown)}>
                {notificationCount != 0 &&
                    <span id="notificationCount" className="text-[10px] text-white absolute w-4 h-4 rounded-sm rounded-full bg-red-600">{notificationCount}</span>}
