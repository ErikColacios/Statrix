...
export default function ReviewSection({ gameReviews, gameId }: Props) {
    ...
    function handleReviewActions(review_id: string) {
        const reviewActionsElement = document.getElementById("reviewActions" + review_id) as HTMLDivElement
        reviewOptionsRef.current = reviewActionsElement
        
        if(reviewOptionsRef.current.classList.contains("hidden")) {
            reviewOptionsRef.current.classList.remove("hidden")
        }else {
            reviewOptionsRef.current.classList.add("hidden")
        }
    }

    const handleClickOutside = (e: MouseEvent) => {
        console.log(reviewOptionsRef.current)
        if (reviewOptionsRef.current && (!reviewOptionsRef.current.contains(e.target as Node))) {
            reviewOptionsRef.current.classList.add("hidden")
        }else {
            reviewOptionsRef.current?.classList.remove("hidden")
        }
                        ...
                        <div className={`flex flex-col overflow-scroll no-scrollbar space-y-2 h-42 p-4 mb-8 rounded-lg bg-black/50 shadow-lg border 
                            ...
                            <div className='relative flex items-center text-white'>
                                ...
                                <div className="flex items-center ml-auto pr-2 text-xs">
                                    ...
                                    <div id={"reviewActions" + review.review_id} className='hidden top-0 right-10 absolute flex flex-col bg-gray-800 p-2 rounded'>
