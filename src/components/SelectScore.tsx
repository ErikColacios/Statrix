...
export default function SelectScore({ score, game_id}:Props){
    ...
    return (
        <div className='flex flex-col items-end sm:flex-row sm:items-center mr-6'>
            ...
            <input id={"score"+game_id} max={10} type="number" className='w-12 pr-1 bg-gray-800 rounded-sm outline-hidden border border border-gray-700 focus:border-green-600 text-right' defaultValue={score} onChange={handleUpdateScore}/>
        </div>
    )
}