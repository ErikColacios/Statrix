            ...
            {listInfo.map((item: any, index: number) => (
                <div className='flex flex-col sm:flex-row' key={index}>
                    <div className='flex flex-col my-6'>
                        ...
                        <div className='flex text-base space-x-8 text-gray-400 mt-2'>
                            {/* Creation date */}
                    ...
                    <div className="flex items-center sm:ml-auto">
                        ...
                        {/* Delete list button*/}
                        <button className="border-green-500 text-green-400 hover:bg-green-900/30 rounded-xl px-6 py-3 text-base">Delete</button>
                        {/* {showModal && 
                ...
                // <div className="relative flex flex-col mt-8 mb-8 md:space-x-32" key={index}>
                //     {/* List name */}
                //     <p className="text-3xl md:text-4xl">{item.list_name}</p>
                //     <div className='flex text-sm text-gray-300 pt-2 pb-4 md:pt-0 md:pb-0 md:text-lg'>
                //         <p>{listContent.length} games</p>
                //     </div>
                //     {/* Creation date */}
                //     <p className='text-base'>Created: {item.list_creationdate.toISOString().split('T')[0]}</p>
                ...
                //         {/* Delete list button*/}
                //         <button className="border-green-500 text-green-400 hover:bg-green-900/30 rounded-xl px-6 py-3 text-lg">Delete</button>
                ...
                {listContent.map((item: any, index: number) => (
                    <div className="relative flex items-center border border-gray-500 rounded rounded-lg overflow-hidden text-sm md:text-lg bg-zinc-900" key={index}>
                        ...
                        <div className='flex flex-col ml-2 sm:ml-10'>
                            <div className='flex'>
                                <Link href={`/gamePage/${item.game_id}`} className="text-lg sm:text-xl mr-4 hover:text-lime-300 hover:underline">{item.game_name}</Link>
