            ...
            <div className="flex flex-col items-center mt-8 mb-6 relative">
                <div className="flex items-center">
                    ...
                    {listInfo.map((item: any, index: number) => (
                        <div key={index} className="mb-6 ml-2 md:ml-12">
                            <p className="mt-1 mb-1">Creation date: {item.list_creationdate.toISOString().split('T')[0]}</p>
                            <input className="w-full md:w-80 xl:w-96 text-black text-2xl p-2 focus:outline-none rounded-e" type="text" id="listName" defaultValue={item.list_name} placeholder="Super list"/>
                        </div>
                    ))}
                </div>
