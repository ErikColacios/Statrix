...
export default function SearchGameBar() {

    let [videogameItems, setVideogameItems] = useState<Videogame[]>([])
    const [gameSearch, setGameSearch] = useState("")
    let [newGamesAdded, setNewGamesAdded] = useState<Videogame[]>([])
    ...
    console.log(newGamesAdded)

    async function removeAddedGames(videogame_id:number){
        console.log("eii" + videogame_id)
        console.log(newGamesAdded)
        setNewGamesAdded(newGamesAdded.filter(item => item.id !== videogame_id))
    }


    /**
     * Adds a new videogame to an array of new games
     * @param videogame 
     */
    async function addVideogame(videogame:Videogame) {
        let gamesAddedDiv = document.getElementById('gamesAdded') as HTMLElement;
        let imageLink:string = `https://images.igdb.com/igdb/image/upload/t_720p/${videogame.cover.image_id}.png`
        let className = `relative flex items-center bg-gray-700 w-full p-1 mb-1 hover:bg-lime-300 hover:text-green-800`


        gamesAddedDiv.innerHTML += `<div class='${className}' ><img src=${imageLink} class='w-8 mr-4' alt='Videogame cover'/><p>${videogame.name}</p>
        <button class='flex items-center w-5 absolute right-0 mr-4' ><img src="/staticImages/icon_remove.png" id='btnRemoveGame' name=${videogame.id} onClick=${console.log("Holaa")}/></button></div>`;
        setNewGamesAdded([...newGamesAdded, videogame])
    }
                        ...
                        <div className="flex flex-col w-full">
                            ...
                            <div className="z-10 w-full text-sm max-h-80 overflow-scroll no-scrollbar" id="gameListSearch" ref={ref}>
                                {videogameItems.map((item:any,index:number) => (
                                    <div className="flex items-center bg-green-500 w-full p-1 mb-1 cursor-pointer hover:bg-lime-300 hover:text-green-800" key={index} onClick={()=> addVideogame(item)}>
