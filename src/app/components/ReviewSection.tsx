...
export default function ReviewSelection({ gameReviews, game_id }: Props) {
                ...
                {reviews?.map((r: any, index: number) => (
                    <div className={`flex flex-col space-y-2 p-4 rounded-lg mb-8 h-32 bg-black/50 shadow-lg border ${r.recommended ? "cardReviewGreen shadow-green-500/30 border-green-600" : "cardReviewRed shadow-rose-500/30 border-rose-700"}`} key={index}>
                        <div className='flex items-center text-white'>
                            ...
                            Reviewed by <b className="hover:text-green-400 ml-1 cursor-pointer">{r.user_name}</b>
                            <span className="ml-8">Likes: <b>{r.likes}</b></span>
                            <span className="ml-8">Review date: {r.review_date.toLocaleDateString()}</span>
                            <button className="flex items-center pr-2 text-xs ml-auto transition bg-gray-800 hover:bg-gray-600 p-1 rounded">
